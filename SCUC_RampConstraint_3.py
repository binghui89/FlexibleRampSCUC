...
def total_cost_objective_rule(m):
   return m.TotalProductionCost + m.TotalFixedCost + m.TotalCurtailmentCost + sum(m.RampingCost[t] for t in m.TimePeriods) + 10000000*(sum(m.MaxWindAvailable[g,t] for g in m.WindGenerators for t in m.TimePeriods) + sum(m.SpinningReserveShortage[t] + m.RegulatingReserveShortage[t] + m.FlexibleRampDnShortage[t] + m.FlexibleRampUpShortage[t] for t in m.TimePeriods))
