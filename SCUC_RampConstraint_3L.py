...
load_df = load_df*load_scaling_factor
...
""" Getting max(Pavail(t+1),Pavail(t))"""

maxavailwindpower_df = genforren_df[wind_generator_names].copy()

for g in wind_generator_names:
    for t in range(23):
        maxavailwindpower_df.loc[t,g] = max(genforren_df.loc[t,g],genforren_df.loc[t+1,g])

sumaxavailwindpower_dict = dict()
for t in range(23):
        sumaxavailwindpower_dict[str(t)] = max(sum(genforren_df.loc[t,wind_generator_names]) , sum(genforren_df.loc[t+1,wind_generator_names]))

sumaxavailwindpower_dict[str(23)] = sum(genforren_df.loc[23,wind_generator_names])
...
model.FlexibleRampUpRequirement = Param(model.TimePeriods, initialize=_flexible_ramp_up_requirement_rule, within=NonNegativeReals, default=0.0, mutable=True)    
model.FlexibleRampDnRequirement = Param(model.TimePeriods, initialize=_flexible_ramp_down_requirement_rule, within=NonNegativeReals, default=0.0, mutable=True)       
...
model.ReserveFactor = Param(within=NonNegativeReals, initialize=ReserveFactor, default=0.0, mutable=True)
...
model.RegulatingReserveFactor = Param(within=NonNegativeReals, initialize=RegulatingReserveFactor, default=0.0, mutable=True)
...
# Fixed status indicator variables for each generator, at each time period.
model.UnitOn = Param(model.ThermalGenerators, model.TimePeriods, within=Binary,initialize=1, mutable=True)
...
def enforce_wind_generator_output_limits_a(m, g, t):
   ...
   return m.PowerGenerated[g,t]    + m.FlexibleRampUpAvailable[g,t]  <= maxavailwindpower_df.loc[t-1,g]


#def enforce_wind_generator_output_limits_aa(m, t):
#    
#   return sum(m.PowerGenerated[g,t]    + m.FlexibleRampUpAvailable[g,t] for g in m.WindGenerators)  <= sumaxavailwindpower_dict[str(t-1)]
...
#model.EnforceWindFlexibleRampUpLimits = Constraint(model.TimePeriods, rule=enforce_wind_generator_output_limits_aa) 
...
def total_cost_objective_rule(m):
   return m.TotalProductionCost + m.TotalFixedCost + m.TotalCurtailmentCost + sum(m.RampingCost[t] for t in m.TimePeriods) + 1000000000000*(sum(m.SpinningReserveShortage[t] + m.RegulatingReserveShortage[t] + m.FlexibleRampDnShortage[t] + m.FlexibleRampUpShortage[t] for t in m.TimePeriods))
