...
def initial_time_periods_online_rule(m, g):
    if not value(m.UnitOnT0[g]):
        ...
        return min(
            ...
            max(0, value(m.MinimumUpHour[g]) - value(m.UnitOnT0State[g]))
        )
...
def initial_time_periods_offline_rule(m, g):
   if value(m.UnitOnT0[g]):
      ...
      return min(
          value(m.NumTimePeriods), 
          max(0, value(m.MinimumDownHour[g]) + value(m.UnitOnT0State[g]))
...
def production_equals_demand_initial_rule(m, t):
    '''
    In other runs, we distinguish energy between power, i.e., m.PowerGenerated[g, t] 
    represents the power level of generator g at the *END* of interval t, while
    the demand is represented as average MW level during interval t, so the 
    total average power of all generators should be greater than the average load.
    In the initalization run, however, we use m.Demand[t] as the instantaneous demand
    instead of average demand over interval t.
    '''
    # if t is m.TimePeriods.first():
    #     p_average = sum((m.PowerGeneratedT0[g] + m.PowerGenerated[g, t])/2 for g in m.AllGenerators)
    # else:
...
def thermal_gen_rampup_rule(m, g, t):
    ramp_per_interval = m.RampUpLimitPerHour[g]/m.nI # m.RampUpLimitPerHour[g] is ramp rate per hour
    if t is m.TimePeriods.first():
...
def enforce_up_time_constraints_subsequent(m, g, t):
    ...
    if i_t <= value(m.InitialTimePeriodsOnLine[g]):
        ...
        return Constraint.Skip
    elif i_t <= (value(m.NumTimePeriods) - value(m.MinimumUpHour[g]) + 1): # Maybe only use one value
        ...
        if t is m.TimePeriods.first():
            return sum(
                ...
                if n >= t and n <= t + value(m.MinimumUpHour[g]) - 1
            ) + m.SlackUpSubsequent_plus[g, t] - m.SlackUpSubsequent_neg[g, t] >= m.MinimumUpHour[g] * (m.UnitOn[g, t] - m.UnitOnT0[g])
        else:
            return sum(
                m.UnitOn[g, n] 
                for n in m.TimePeriods 
                if n >= t and n <= t + value(m.MinimumUpHour[g]) - 1
            ) + m.SlackUpSubsequent_plus[g, t] - m.SlackUpSubsequent_neg[g, t] >= m.MinimumUpHour[g] * (m.UnitOn[g, t] - m.UnitOn[g, m.TimePeriods.prev(t)])
    else:
        # Handle the final (MinimumUpHour[g] - 1) time periods - if a unit is 
...
def enforce_down_time_constraints_subsequent(m, g, t):
    ...
    if i_t <= value(m.InitialTimePeriodsOffLine[g]):
        ...
        return Constraint.Skip
    elif i_t <= (value(m.NumTimePeriods) - value(m.MinimumDownHour[g]) + 1):
        ...
        if t is m.TimePeriods.first():
            return sum(
                1 - m.UnitOn[g, n]
                for n in m.TimePeriods 
                if n >= t and n <= t + value(m.MinimumDownHour[g]) - 1
            ) + m.SlackDnSubsequent_plus[g, t] - m.SlackDnSubsequent_neg[g, t] >= m.MinimumDownHour[g] * (m.UnitOnT0[g] - m.UnitOn[g, t])
        else:
            return sum(
                1 - m.UnitOn[g, n] 
                for n in m.TimePeriods 
                if n >= t and n <= t + value(m.MinimumDownHour[g]) - 1
            ) + m.SlackDnSubsequent_plus[g, t] - m.SlackDnSubsequent_neg[g, t] >= m.MinimumDownHour[g] * (m.UnitOn[g, m.TimePeriods.prev(t)] - m.UnitOn[g, t])
    else:
        # handle the final (MinimumDownHour[g] - 1) time periods - if a unit is 
...
def production_cost_initial_function(m, g, t):
    '''
    This function comes from normal run's production_cost_function. Note that in 
    the normal run, the production cost has a unit of $/h, however, in the 
    initialization model we are not looking at a time interval, so there is no 
    time interval.
    '''
    return m.ProductionCost[g,t] == (
        m.UnitOn[g,t]*m.BlockMarginalCost0[g]
...
def compute_total_curtailment_cost_initial_rule(m):
    '''
    Adapted from compute_total_curtailment_cost_rule in the normal run.
    Similar to the production cost model in the normal run, we don't have a time
    interval in the initialization model so no time interval compared with normal run.
    '''
    return m.TotalCurtailmentCost == sum(
        m.BusVOLL[b] * m.BusCurtailment[b,t]
        for b in m.LoadBuses 
...
# Objective of initialization model
def total_cost_inital_objective_rule(m):
    '''
    Adapted from the objective function in the normal run, but without fixed cost
    and penalty cost for reserve and load imbalance.
    '''
    return (
        m.TotalProductionCost 
        # + m.TotalFixedCost 
...
def EnforceGeneratorOutputLimitsDispacthLower_rule(m, g, t):
    return m.MinimumPowerAvailable[g, t] >= m.DispatchLimitsLower[g, t]

def create_model_initial(
    network,
    df_busload,
    ...
    ''' 
    Initialization model: to determine the initial generator commitment 
    status and generation level.
    '''
    ...
    # model.RampUpLimitPerHour = Param(
    #     model.ThermalGenerators,
    #     within=NonNegativeReals,
    #     initialize=network.dict_rampup_by_gen,
    # )
    # model.RampDownLimitPerHour = Param(
    #     model.ThermalGenerators,
    #     within=NonNegativeReals, 
    ...
    # )
    # if dict_UnitOn:
    #     for k in dict_UnitOn.iterkeys():
    #         model.UnitOn[k].fix(dict_UnitOn[k])
    #         model.UnitStartUp[k].fix(dict_UnitStartUp[k])
    #         model.UnitShutDn[k].fix(dict_UnitShutDn[k])
    ...
    model.ProductionEqualsDemandInitial = Constraint(
        model.TimePeriods, rule=production_equals_demand_initial_rule
    )
    ...
    model.all_gen_output_max_available = Constraint(
        model.AllGenerators,
        model.TimePeriods, 
        rule=all_gen_output_max_available_rule,
    )
    model.all_gen_output_min_available = Constraint(
        model.AllGenerators,
        model.TimePeriods, 
    ...
    model.ComputeProductionCost = Constraint(
        model.ThermalGenerators, 
        model.TimePeriods, 
        rule=production_cost_initial_function
    )
    ...
    model.ComputeTotalCurtailmentCost = Constraint(rule=compute_total_curtailment_cost_initial_rule)
    ...
    model.TotalCostObjective = Objective(
        rule=total_cost_inital_objective_rule, sense=minimize
    )
    ...
    #         model.ThermalGenerators_commit,
    #         model.TimePeriods,
    #         within=NonNegativeReals, 
    #         initialize=dict_DispatchLimitsUpper,
    #     )
    #     model.DispatchLimitsLower = Param(
    #         model.ThermalGenerators_commit,
    #         model.TimePeriods,
    #         within=NonNegativeReals, 
    #         initialize=dict_DispatchLimitsLower,
    #     )
    #     model.EnforceGeneratorOutputLimitsDispacthUpper = Constraint(
    #         model.ThermalGenerators_commit,
    #         model.TimePeriods,
    #         rule=EnforceGeneratorOutputLimitsDispacthUpper_rule
    #     )
    #     model.EnforceGeneratorOutputLimitsDispacthLower = Constraint(
    #         model.ThermalGenerators_commit,
    #         model.TimePeriods,
    #         rule=EnforceGeneratorOutputLimitsDispacthLower_rule
    #     )
...
def test_dauc(casename, showing_gens='problematic'):
    ...
    # End of DAUC
    ############################################################################
    df_results_initial = pd.DataFrame(np.nan, index=instance.AllGenerators, columns=['UnitOn', 'MinimumPowerOutput', 'MinimumPowerAvailable', 'PowerGenerated', 'MaximumPowerAvailable', 'MaximumPowerOutput'])
    for g in instance.AllGenerators:
        df_results_initial.at[g, 'MinimumPowerAvailable'] = value(instance.MinimumPowerAvailable[g, 1])
        df_results_initial.at[g, 'PowerGenerated'] = value(instance.PowerGenerated[g, 1])
        df_results_initial.at[g, 'MaximumPowerAvailable'] = value(instance.MaximumPowerAvailable[g, 1])
        if g in instance.ThermalGenerators:
            df_results_initial.at[g, 'MinimumPowerOutput'] = value(instance.MinimumPowerOutput[g])
            df_results_initial.at[g, 'MaximumPowerOutput'] = value(instance.MaximumPowerOutput[g])
            df_results_initial.at[g, 'UnitOn'] = value(instance.UnitOn[g, 1])
